$.getScript('audiorecorder.min.js', function() {
	
	var socket = io();	
	
	var config = { 'worker_path': 'worker.min.js' }
	...
	function captureMicrophone() {
		...
		const processAudio = ev => {
			...
			socket.emit('clientSendBuffor', encodedData);
		};
	...
	socket.on('serverSendBuffor', function(data){
